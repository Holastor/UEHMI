...
// Pre-encode reference audio to acoustic tokens that can be reused across
// multiple ov_synthesize calls via the ref_audio_tokens / ref_T fields.
// audio_24k: mono float PCM at 24 kHz (same format as ref_audio_24k).
// n_samples: length in samples.
// tokens_out: receives malloc-allocated flat int32 array (K * T elements,
//   row-major with K codebooks and T time steps). Caller must free with
//   ov_audio_tokens_free.
// T_out: receives the number of time steps T (K is internal, typically 8).
// Returns OV_STATUS_OK on success. Requires a codec-loaded handle.
OV_API enum ov_status ov_encode_reference_audio(struct ov_context * ov,
                                                 const float * audio_24k,
                                                 int           n_samples,
                                                 int32_t **    tokens_out,
                                                 int *         T_out);

// Free tokens allocated by ov_encode_reference_audio. Safe on nullptr.
OV_API void ov_audio_tokens_free(int32_t * tokens);
